...
let JwtStrategy = class JwtStrategy extends (0, passport_1.PassportStrategy)(passport_jwt_1.Strategy) {
    ...
    async validate(payload) {
        ...
        if (!isSessionActive) {
            throw new common_1.UnauthorizedException('Sesi telah berakhir atau tidak aktif. Silakan masuk kembali.');
        }
        ...
        if (!user || !user.is_active) {
            throw new common_1.UnauthorizedException('Akun Anda dinonaktifkan atau tidak ditemukan.');
        }
        return {
            id: user.id,
            email: user.email,
